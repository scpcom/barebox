...
static int c2000_device_init(void)
{
	...
	writel( readl(COMCERTO_GPIO_OE_REG)     |  (0x33e0), COMCERTO_GPIO_OE_REG);		// Sequoia: Enable LED-GPIOs Output
	writel( readl(COMCERTO_GPIO_OUTPUT_REG) |  (0x33e0), COMCERTO_GPIO_OUTPUT_REG);	        // Sequoia: Turn ON LED
#endif
	...
	// Western Digital: Sequoia, for now...
	// First, spin up drive power...
	writel(readl(COMCERTO_GPIO_32_PIN_SELECT_REG) | GPIO_51, COMCERTO_GPIO_32_PIN_SELECT_REG);   // GPIO pin.
	writel(readl(COMCERTO_GPIO_32_OE_REG) & ~GPIO_51, COMCERTO_GPIO_32_OE_REG);                  // Make output.
	writel(readl(COMCERTO_GPIO_32_OUTPUT_REG) | GPIO_51, COMCERTO_GPIO_32_OUTPUT_REG);           // Assert high.
	...
	if(!bootopt){
		...
		// Western Digital: Put env at sector 1 for 1 sector. uLoader is at sector 0, barebox starts at sector 2...
		// devfs_add_partition("spi0", 0xA0000, 0x20000, PARTITION_FIXED, "env0");
		devfs_add_partition("spi0", 0x10000, 0x10000, PARTITION_FIXED, "env0");
		protect_file("/dev/env0", 1);
	}else{
		/* ENV for NOR */
		printf("Using ENV from NOR Flash.\n");
		devfs_add_partition("nor0", 0x60000, 0x20000, PARTITION_FIXED, "env0");
		protect_file("/dev/env0", 1); //pratap has to comment
	}
...
int c2000_eth_board_init(int gemac_port)
{
	...
	if(gemac_port == 0)
	{
		...
		// Western Digital: Moved to above...
		// Western Digital: Keep here because eth0 was sometimes not working?
		writel(readl(COMCERTO_GPIO_OE_REG) | GPIO_27, COMCERTO_GPIO_OE_REG);
		writel(readl(COMCERTO_GPIO_OUTPUT_REG) | GPIO_27, COMCERTO_GPIO_OUTPUT_REG);
		udelay(10000);
                writel(readl(COMCERTO_GPIO_OUTPUT_REG) & ~GPIO_27, COMCERTO_GPIO_OUTPUT_REG);
		...
		//Western Digital: No Atheros in Sequoia...
		//AR8327 Switch init
		//athrs17_init(mdev);
