...
static int is_ready;
struct ahci_probe_ent ahci_probe_ent_g;
...
struct sata_port_regs *g_port0_mmio = NULL;
...
static int ahci_host_init(struct ahci_probe_ent *probe_ent)
{
	...
	int clk = HAL_get_axi_clk() * 1000 * 1000; 
	...
	for (i = 0; i < probe_ent->n_ports; i++) {
		...
		for (j = 0; j < 100; j++) {
			...
			//No drive is connected to this port
			if (tmp == 0x0)
				break;
		}

		debug_sata("SSTS=%08x, loop_count: %d\n", tmp, j);
		if (j == 100)
		{
			debug_sata("SSTS timeout!\n");
			return 2;
		}
...
/* to send send_by_immediate command */
int sleep_sata(int dev)
{
	struct ahci_probe_ent *probe_ent =
		(struct ahci_probe_ent *)sata_dev_desc[dev].priv;
	struct sata_fis_h2d h2d, *cfis = &h2d;
	...
	debug_sata("sending standby-immediate cmd:\n");
